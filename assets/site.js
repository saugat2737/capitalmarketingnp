...
function renderFooterCategories() {
  ...
  list.innerHTML =
    categories
      ...
      .map(
        (category) =>
          `<li><a href="${pathPrefix()}categories/${category.slug}.html">${category.name}</a></li>`
      )
      .join("") +

    `<li>
      <a href="${pathPrefix()}products.html" style="font-weight:700;">
        More Products →
      </a>
    </li>`;
}
...
function renderCategoryPage() {
  ...
  if (category.slug === "event-branding-materials") {
  gallery.innerHTML = `
    <div style="grid-column: 1 / -1; margin-bottom: 30px;">
      <img
        src="../assets/images/products/event-branding-materials-banner.jpg"
        alt="Premium Corporate T-Shirts"
        style="width:100%; border-radius:16px; object-fit:cover;">
    </div>
  `;
}
  const names = category.slug === "pens"
  ? [
      "Signature Edition",
      "Executive Series",
      "Classic Corporate",
...
gallery.innerHTML += names.map((name, index) => `
